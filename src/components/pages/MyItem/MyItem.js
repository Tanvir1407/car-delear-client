...
const MyItem = () => {
  ...
  useEffect(() => {
    // fetch(`https://pure-stream-67963.herokuapp.com/myitem?email=${userEmail}`, {
    //   headers: {
    ...
    const getMyItems = async () => {
      ...
      const url = `https://pure-stream-67963.herokuapp.com/myitem?email=${userEmail}`;
  ...
  const handleDeleteItem = (id) => {
    ...
    setProceed(proceed);
    if (proceed) {
      fetch(`https://pure-stream-67963.herokuapp.com/delete/${id}`, {
        method: "DELETE",
      })
        .then((res) => res.json())
        .then((data) => {
